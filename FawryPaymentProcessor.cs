...
using Nop.Plugin.Payments.Fawry.Services;
using Nop.Plugin.Payments.Fawry.Validators;
...
{
    public class FawryPaymentProcessor : BasePlugin, IPaymentMethod
    {
        #region Fields
        ...
        private readonly FawryHttpClient _fawryHttpClient;
        ...
        public FawryPaymentProcessor(CurrencySettings currencySettings,
            ...
            FawryHttpClient fawryHttpClient,
            ...
            IEncryptionService encryptionService)
        {
            ...
            _fawryHttpClient = fawryHttpClient;
        ...
        {
            response = WebUtility.UrlDecode(_fawryHttpClient.GetPdtDetailsAsync(tx).Result);
        ...
        {
            var response = WebUtility.UrlDecode(_fawryHttpClient.VerifyIpnAsync(formString).Result);
        ...
        private async Task<IDictionary<string, string>> CreateQueryParameters(PostProcessPaymentRequest postProcessPaymentRequest)
        {
            ...
            return new Dictionary<string, string>
            {
                ...
                ["bn"] = FawryHelper.NopCommercePartnerCode,
                ...
                ["return"] = $"{storeLocation}Plugins/PaymentFawry/PDTHandler",
                ["notify_url"] = $"{storeLocation}Plugins/PaymentFawry/IPNHandler",
                ["cancel_return"] = $"{storeLocation}Plugins/PaymentFawry/CancelOrder",
        ...
        private async void AddItemsParameters(IDictionary<string, string> parameters, PostProcessPaymentRequest postProcessPaymentRequest)
        {
          ...
          await  foreach (var (attribute, values) in checkoutAttributeValues)
           ...
           await _genericAttributeService.SaveAttributeAsync(postProcessPaymentRequest.Order, FawryHelper.OrderTotalSentToFawry, roundedCartTotal);
        }
        ...
        private async void AddOrderTotalParameters(IDictionary<string, string> parameters, PostProcessPaymentRequest postProcessPaymentRequest)
        {
           ...
           await _genericAttributeService.SaveAttributeAsync(postProcessPaymentRequest.Order, FawryHelper.OrderTotalSentToFawry, roundedOrderTotal);
        }
        ...
        {
            if (_fawryPaymentSettings.UseSandbox)
            {
                ...
                TimeSpan diff = DateTime.UtcNow.AddHours(_fawryPaymentSettings.PaymentExpiry) - baseDate;
                long paymentExpiry = Convert.ToInt64(diff.TotalSeconds);
                ...
                var baseUrl = "https://atfawry.fawrystaging.com/fawrypay-api/api/payments/init";
                var ReturnUrl = storeUrl + "Plugins/PaymentFawry/PDTHandler";
                HttpClient client = new HttpClient();
                var concat = "1tSa6uxz2nR3D19knydXKg==" + postProcessPaymentRequest.Order.Id + "1212" + ReturnUrl + ProductId + Math.Round(postProcessPaymentRequest.Order.OrderTotal, 2) + "d6413b9273fe478ba7f42283c7b5355b";
                var hash = ComputeSha256Hash(concat);

                var values = new[] {
                new {
                    itemId=ProductId,
                    price=Math.Round(postProcessPaymentRequest.Order.OrderTotal, 2),
                    quantity= 1
                ...
                var trandata = new
                {
                    paymentExpiry = paymentExpiry * 1000,
                    merchantCode = "1tSa6uxz2nR3D19knydXKg==",
                    merchantRefNum = postProcessPaymentRequest.Order.Id,
                ...
                {
                    _httpContextAccessor.HttpContext.Response.Redirect(responseString);
                }
            }
            else
            {
                ...
                var webHelper = Nop.Core.Infrastructure.EngineContext.Current.Resolve<Nop.Core.IWebHelper>();
                var storeUrl = webHelper.GetStoreLocation();
                var baseUrl = "https://www.atfawry.com/fawrypay-api/api/payments/init";
                var ReturnUrl = storeUrl + "Plugins/PaymentFawry/PDTHandler";
                HttpClient client = new HttpClient();
                var concat = _fawryPaymentSettings.MerchantCode + postProcessPaymentRequest.Order.Id + "1212" + ReturnUrl + ProductId + Math.Round(postProcessPaymentRequest.Order.OrderTotal, 2) + _fawryPaymentSettings.SecurityKey;
                var hash = ComputeSha256Hash(concat);
                ...
                {
                    paymentExpiry = paymentExpiry * 1000,
                    merchantCode = _fawryPaymentSettings.MerchantCode,
                    merchantRefNum = postProcessPaymentRequest.Order.Id,
                    customerProfileId = "1212",
                    chargeItems = values,
                    paymentMethod = "",
                ...
                var log = EngineContext.Current.Resolve<Nop.Services.Logging.ILogger>();
                await log.InsertLogAsync(Nop.Core.Domain.Logging.LogLevel.Information, "Fawry", str);
                ...
                if (!string.IsNullOrEmpty(responseString) && responseString.Contains("https://www.atfawry.com/atfawry/plugin/?payment-id"))
                {
                    _httpContextAccessor.HttpContext.Response.Redirect(responseString);
                }
                else
                {
                    _httpContextAccessor.HttpContext.Response.Redirect(storeUrl);
                }
            }
        ...
        {
            return $"{_webHelper.GetStoreLocation()}Admin/PaymentFawry/Configure";
        }
        ...
        {
            await _settingService.SaveSettingAsync(new FawryPaymentSettings
            {
                UseSandbox = true
            });
            ...
            {
                ["Plugins.Payments.Fawry.Fields.MerchantCode"] = "MerchantCode",
                ["Plugins.Payments.Fawry.Fields.MerchantCode.Hint"] = "Enter Merchant Code.",
                ["Plugins.Payments.Fawry.Fields.PaymentExpiry"] = "Payment Expiry",
                ["Plugins.Payments.Fawry.Fields.PaymentExpiry.Hint"] = "Enter Payment Expiry.",
                ["Plugins.Payments.Fawry.Fields.SecurityKey"] = "Security Key",
                ["Plugins.Payments.Fawry.Fields.SecurityKey.Hint"] = "Enter Security Key.",
                ["Plugins.Payments.Fawry.Fields.RedirectionTip"] = "You will be redirected to Fawry site to complete the order.",
                ["Plugins.Payments.Fawry.Fields.UseSandbox"] = "Use Sandbox",
                ["Plugins.Payments.Fawry.Fields.UseSandbox.Hint"] = "Check to enable Sandbox (testing environment).",
            });
        ...
        {
            await _settingService.DeleteSettingAsync<FawryPaymentSettings>();

            await _localizationService.DeleteLocaleResourcesAsync("Plugins.Payments.Fawry");
        ...
        {
            return await _localizationService.GetResourceAsync("Plugins.Payment.Fawry.PaymentMethodDescription");
        ...
        public  Task<string> PaymentMethodDescription => _localizationService.GetResourceAsync("Plugins.Payments.Fawry.PaymentMethodDescription");
